...
//index function
    public function CSV_file24()
    {
        ...
        $excel_file = $_FILES['file22']['name'];
        $file22_temp = $_FILES['file22']['tmp_name'];
        ...
        if ($file_type == 'pdf') {
            ...
            $local = "./documents/" . $actual_name . '.xlsx';
            $this->load->library("Csvconverter");
            $this->csvconverter->convert($pdf_file, $actual_name);
            ...
            //     // $apikey = 'qql7sj16uwon'; // from https://pdftables.com/api

            //     //$apikey = 'qql7sj16uwon'; UNUSED
...
/////////////////////////////////////////////////////////////////////////
                ...
                $data_to_insert = array(
                    ...
                    'validitytime' => "6:00pm -6:00pm (24hrs)",
            ...
            //echo "Excel Uploader";exit();
            $this->Excel_24_uploader($excel_file, $file22_temp);
        }

    }

    public function Excel_24_uploader($filename, $file_temp)
    {

        //$filename = $_FILES['file2']['name'];
        $dir = '/var/www/html/wids/Conversions/' . $filename;
        move_uploaded_file($file_temp, $dir);
    ...
    public function Excel_6_uploader($filename, $file_temp)
    {
        ...
        if ($xlsx = SimpleXLSX::parse($local)) {
            ...
            $data = array(
                ...
                'wind_dir' => $wind_dir,
                'wind_str' => $wind_str,
                'forecaster' => $_SESSION['first_name'] . ' ' . $_SESSION['last_name'],
                'regs' => $min,
                'change' => 91,
            );
    ...
    public function index()
    {
        ...
        if ($file_type == 'pdf') {
            ...
            //         $c = curl_init();
            //         $cfile = curl_file_create($pdf_file, 'application/pdf');
            //     //--------- b4578c7kr3fi calling the conversion api   ---------------------------------------
            //     $apikey = '4fzssxnbw3zm'; // from https://pdftables.com/api
            //     curl_setopt($c, CURLOPT_URL, "https://pdftables.com/api?key=$apikey&format=xlsx-single"); curl_setopt($c, CURLOPT_POSTFIELDS, array('file' => $cfile)); curl_setopt($c, CURLOPT_RETURNTRANSFER, true); curl_setopt($c, CURLOPT_FAILONERROR,true); curl_setopt($c, CURLOPT_ENCODING, "gzip,deflate");  $result = curl_exec($c);

            //     if (curl_errno($c) > 0) {
            //        // $data['change'] = 3;

            //         $data['alert'] = "PDF Conversion Units are done with apikey = ". $apikey;

            //         $this->session->set_flashdata('message','<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button> Insufficient PDF Conversion Units! <b>PLEASE USE THE EXCEL UPLOADER</b> </div>');

            //         redirect('index.php/daily_forecast/pdf_uploader');
            ...
            if ($xlsx = SimpleXLSX::parse($local)) {
                ...
                for ($m = 0; $m < sizeof($temp); $m++) {
                    $wet_cat = ($this->CSV_model->get_weather_cat(trim($weather_desc[$m]))->id == "") ? 10 : $this->CSV_model->get_weather_cat(trim($weather_desc[$m]))->id;
                    $data_to_insert2 = array(
                        'mean_temp' => (double)substr($temp[$m], 0, strlen($temp[$m] - 1)),
                        'max_temp' => (double)substr($temp[$m], 0, strlen($temp[$m] - 1)),
                        'min_temp' =>(double) substr($temp[$m], 0, strlen($temp[$m] - 1)),
                        'wind' => (double)substr($temp[$m], 0, strlen($temp[$m] - 1)),
                        'wind_direction' => $wind_dir[$m],
                        'wind_strength' => $wind_str[$m],
                        'region_id' => ($m + 1),
                        'weather_cat_id' => ($wet_cat == "") ? 10 : $wet_cat,
            ...
            //echo "Excel Uploader";exit();
            $this->Excel_6_uploader($excel_file, $file22_temp);
        }
    }
