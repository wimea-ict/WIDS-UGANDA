...
{ 
 ...
 $data = array('major_data' => $major_sector,'change' => 61);
...
public function saveMajorSector(){
    ...
    if (!is_null($id) && $this->Major_model->get_by_id($id)) {
        ...
        $this->session->set_flashdata('message','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>Major Sector record updated Successfully!.</div>');
    } else {
        ...
        $this->session->set_flashdata('message','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>Major Sector record added Successfully!.</div>');
    }
	
    redirect('index.php/Major_Sector');
}
...
public function update($id=NULL) 
{
   $row = $this->Major_model->get_by_id($id);
    ...
    public function word()
    {
        ...
        $data = array('major_data'=> $this->Major_model->get_all(),'start' => 0);
        $this->load->view('major_sector_doc',$data);
    }

    public function delete()
    {
        $this->Major_model->delete($this->uri->segment(3));
        $this->session->set_flashdata('message','<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>Major Sector record deleted Successfully!.</div>');
        redirect('index.php/Major_Sector');
    }
