...
class Daily_forecast extends CI_Controller
{
    ...
    {   $id = $this->uri->segment(3);
        $data['change'] =3;
            $this->Daily_forecast_model->delete1($id);
               ...
               $this->session->set_flashdata('message','<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>Record Deleted Successfully!</div>');

            redirect("index.php/Daily_forecast/");
    ...
    public function save_update(){
        ...
        $this->session->set_flashdata('message','<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>Record Updated Successfully!</div>');
...
return $result;
         ...
         public function wording()
    {
        ...
        header("Content-Disposition: attachment;Filename=Area daily forecast data.doc");
        $data = array(
            'daily_forecast_data' => $this->Daily_forecast_model->get_daily_forecast_data($_SESSION['daily_forecast_data']),
            'start' => 0
        );
    ...
    public function pdfing()
    {
        ...
        $html = $this->load->view('daily_forecast_data2', $data, true);
        $this->load->library('pdf');
        $pdf = $this->pdf->load();
        $pdf->WriteHTML($html);
        $pdf->Output('Area daily forecast data.pdf', 'D');
    }
...
function daily_forecast_data(){
  $id = $this->uri->segment(3);
  $_SESSION['daily_forecast_data'] = $this->uri->segment(3);
  $data['daily_forecast_data'] = $this->Daily_forecast_model->get_daily_forecast_data($id);
  $country = $this->config->item('country');
  $data['division_type']= $this->Division_model->getdivisionname($country);  
  $data['forecast_id'] = $id;

  $data['identity'] = $this->Daily_forecast_model->get_period($id);
  $data['change'] = 78; 
   $data['button'] = "SUBMIT";
  $this->load->view('template', $data);	
...
function saveforecastdata24(){
    ...
    if($exists == TRUE){
         $this->session->set_flashdata('message','<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>'.$k['region_name'].' Forecast data already exists!</div>');

        $data['forecast_id' ] = $this->input->post('forecast_id');
    }else{
        
        for ($i=1; $i < 5; $i++) { 
            $data = array(
                    'weather_cat_id' => $this->input->post('cat_id'.$i,TRUE),
                    'mean_temp' => $this->input->post('mean_temp'.$i,TRUE),
            ...
            redirect('index.php/Daily_forecast/daily_forecast_data/'.$_SESSION['rec_id']);
            //$this->load->view('template',$data);     
  ...
  public function save(){
    ...
    $date_issue = date('Y',$issuedate).'-'.date('m',$issuedate).'-'.date('d',$issuedate);
      //-------end of date format----------------
         ...
         $exists = FALSE;
        // print_r($recs); echo sizeof($recs);   exit();
         foreach ($recs as $key) {
             $exists = TRUE;
         }
    	 ...
    	 $this->session->set_flashdata('message','<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button> Forecast data already exists!</div>');
    }
    redirect("index.php/Daily_forecast");
	}
//single form create action
    ...
    {   $id = $this->uri->segment(3);
        $data['change'] =3;
            ...
            $this->session->set_flashdata('message', '<font color="red" size="5">'.$this->session->userdata('deleted').' Forecast Data Deleted Successfully!</font>');
            $this->index();
