...
class Daily_forecast_model extends CI_Model
{
    ...
    function delete($id)
    {
       ...
       $this->db->join('region','region.id = daily_forecast_data.region_id');
        ...
        $ret = $this->db->get()->row();
        $this->session->set_userdata('deleted',$ret->region_name);
    ...
    function forecast_data_checker($city_id, $forecast_id)
    {
      $this->db->select('region_id,region_name');
      $this->db->from('daily_forecast_data');
      $this->db->join('region','region.id = daily_forecast_data.region_id');
      $this->db->where('region_id', $city_id);
    ...
    function get_all()
    {
      $this->db->select('daily_forecast.id,daily_forecast.date,daily_forecast.weather,forecast_time.period_name as time,daily_forecast.issuedate,daily_forecast.validitytime, daily_forecast.dutyforecaster, ussdmenulanguage.language');
      $this->db->from('daily_forecast');
      $this->db->join('ussdmenulanguage','daily_forecast.language_id = ussdmenulanguage.id');
    ...
    function forcast_for_24hrs($division_id){
      $this->db->select('weather_category.svg_data, daily_forecast.issuedate,division.division_name, daily_forecast.validitytime,daily_forecast.weather,
        ...
        weather_category.cat_name, weather_category.img,daily_forecast_data.period as timing, daily_forecast.date as forecasted');
    $this->db->from('daily_forecast'); 
    ...
    $this->db->where("daily_forecast.time", 5); 

    //-----------------new change--------------------
    //$this->db->limit(4);
    
   // $this->db->order_by('daily_forecast.date', 'DESC');
...
function get_daily_forecast_data_for_region1($forecast_id,$division_id){
	...
	$this->db->select('daily_forecast_data.id, daily_forecast_data.mean_temp,daily_forecast_data.max_temp,daily_forecast_data.min_temp,daily_forecast_data.wind,daily_forecast_data.wind_direction,daily_forecast_data.wind_strength,division.division_name,region.region_name,weather_category.cat_name,daily_forecast_data.datetime,daily_forecast_data.forecast_id,forecast_time.period_name as time,daily_forecast.');
	$this->db->from(' daily_forecast_data');	
   ...
   function get_daily_forecast_data_for_region($forecast_id,$division_id){
    $this->db->select('daily_forecast.issuedate,daily_forecast.date, forecast_time.period_name as time,division.division_name, daily_forecast.validitytime,daily_forecast.weather,
        daily_forecast_data.max_temp,daily_forecast_data.min_temp, daily_forecast_data.mean_temp, daily_forecast_data.wind, daily_forecast_data.wind_direction, daily_forecast_data.wind_strength ,daily_forecast.datetime,
   ...
   $this->db->join('forecast_time','daily_forecast.time = forecast_time.id');
    ...
    $this->db->where_not_in("daily_forecast.time", 5); 
     $this->db->order_by('forecast_time.id',"ASC"); 
    // $this->db->where("daily_forecast.id", $forecast_id);    
   ...
   $this->db->join('forecast_time','daily_forecast.time = forecast_time.id');
    ...
    $this->db->where("division.id", $division_id); 
    $this->db->where("daily_forecast.language_id", 1); 
    $this->db->where_not_in("daily_forecast_data.period", 5); 
