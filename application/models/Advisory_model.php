...
function get_regions(){
    ...
    function check_existence( $f_id, $sect, $reg){
            $this->db->select('id');
            $this->db->from('advisory');
            $this->db->where('sector', $sect);
...
//----------------------end of health and water adviory functions---------------------------
    ...
    {   
        $this->db->select('sub_region.sub_region_name, minor_sector.minor_name as sector_name, advisory.id,advisory.sector,advisory.forecast_id,advisory.advice,advisory.message_summary,seasonal_forecast.season_id,seasonal_forecast.year,season_months.abbreviation, ussdmenulanguage.language');
        $this->db->from('advisory');
        $this->db->join('seasonal_forecast','advisory.forecast_id=seasonal_forecast.id');
        $this->db->join('season_months','seasonal_forecast.season_id=season_months.id'); 
