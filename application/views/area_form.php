      ...
      <tr><td>Region <?php echo form_error('region_id') ?></td>
           <td> 
           <select name="region_id" id="main_region"  class="form-control" id="region_id">
      ...
      <tr>
        ...
        <td><textarea class="form-control" rows = "5" name="overall_comment" id="overall_comment" placeholder="Overall Comment" /> <?php echo $overall_comment; ?></textarea></td>
     </tr>
      <!-- <tr > -->
      ...
      <a href="<?php echo site_url('index.php/Season/showareaforecast').'/'.$this->uri->segment(3) ?>" class="btn btn-default">Cancel</a></td></tr>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#main_region').change(function(){ 
                var id=$(this).val();
                $.ajax({
                    url : "<?php echo base_url().'index.php/Product/get_sub_region'?>",
                    method : "POST",
                    data : {id: id},
                    // async : true,
                    dataType : 'json',
                    success: function(data){
                        ...
                        var html = '<select class="form-control" name="subregion_id" id="subregion_id">';
                        var i;
                        for(i=0; i<data.length; i++){
                          html +='<option value="';
                          html += data[i].id;
                          html +='">';
                          html += data[i].sub_region_name;
                          html +='</option>';   
                        }
                        html +='</select>';
                        $('#sector').html(html);
